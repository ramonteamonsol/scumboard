            ...
            >
              ...
              <p
                className={`${activeIndex === i ? 'block' : 'hidden'
                  } font-semibold sm:font-bold text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl !leading-[129%] text-white font-helvetica-neue-condensed pb-1 md:pb-2 xl:pb-6 max-w-[1359px]`}
              >
                {faq.answer.map((ans, index) =>
                  ans.href ? (
                    <Link
                      key={index}
                      href={ans.href}
                      className={`inline-block text-[#E04B77] font-helvetica-neue-condensed`}
                      target="_blank"
