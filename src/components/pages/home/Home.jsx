...
const Home = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col"> {/* Ensures full height and flex layout */}
      <div 
        ref={containerRef} 
        className="bg-[url('/assets/webp/wall.webp')] bg-no-repeat [background-size:200%_100%] sm:[background-size:150%_100%] md:[background-size:100%_100%]  w-full max-w-full overflow-auto lg:overflow-hidden flex-1"
      >
        {!isAtStart && (
          <button
            onClick={scrollLeft}
        ...
        <div className='h-full flex justify-end items-end min-w-[1024px] relative overflow-hidden'>
          <Image
            ...
            width={1500}
            height={600}
            className="w-full absolute -bottom-20 left-[67%] max-w-[250px] xl:max-w-[300px] z-50 transition-all ease-out duration-100 cursor-pointer"
          />
          <div
            style={{ transform: `translateX(${objectPos}px)` }}
