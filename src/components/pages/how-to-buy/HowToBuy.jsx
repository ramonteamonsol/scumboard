        ...
        <div className="space-y-5 lg:space-y-7 2xl:space-y-9 select-none pr-5 sm:pr-7 md:pr-10 lg:pr-14 xl:pr-20 2xl:pr-28 overflow-hidden">
          {HowToBuyData.map((que, i) => (
            ...
            >
                ...
                <div className="pt-1 pb-2 xl:py-0">
                  ...
                  <p
                    className={`font-semibold sm:font-bold text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-2xl !leading-[129%] ${que.indexClassName || "text-white"
                      } font-helvetica-neue-condensed pb-1 md:pb-2 xl:pb-6 max-w-[1359px]`}
                  >
                    {que.answer.map((ans, index) =>
                      ans.href ? (
                        ...
                        <span className='font-helvetica-neue-condensed' key={index}>{ans.text}</span>
                      )
                    )}
                  </p>
                </div>
              ...
              {que.img && (
                <div className="hidden md:flex items-center justify-center border-2 xl:border-4 border-[#E04B77] rounded-3xl lg:rounded-[45px] xl:rounded-[69px] overflow-hidden md:w-full h-full max-w-[200px] xl:max-w-[300px] max-h-[300px]">
                  <Image src={que.img} alt="baron money low" width={300} height={300} className='w-full h-auto' />
                </div>
              )}
            </div>
          ))}
